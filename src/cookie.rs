...
//!
//! Intentionally out of scope: IDN normalisation, the Public Suffix List
//! (Domain cookie rejection of `Domain=co.uk`), eTLD handling, and SameSite
//! enforcement. None of these matter for a CLI tool that follows a single
//! user-driven redirect chain.
...
impl CookieJar {
    ...
    pub fn load_netscape(path: &str) -> Result<Self> {
        ...
        for (lineno, line) in BufReader::new(f).lines().enumerate() {
            ...
            {
                // Malformed non-comment line — surface as a soft warning
                // through Err so the caller can decide. We pick BadResponse
                // because there's no dedicated cookie error variant.
...
/// Parse one line of a Netscape `cookies.txt` file. Returns `None` for
/// blank lines and pure-comment lines; otherwise a `Cookie` or `None` if
/// the field count or types are wrong.
fn parse_netscape_line(line: &str) -> Option<Cookie> {
    ...
    let expires_raw: i64 = fields[4].parse().ok()?;
